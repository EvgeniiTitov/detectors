...
from yolov4_tiny.tool.utils import *
from yolov4_tiny.tool.darknet2pytorch import Darknet
import os
import torch
import numpy as np
import cv2
...
class YOLOv4Tiny:
    path_to_dependencies = os.path.join(os.getcwd(), "yolov4_tiny", "dependencies", f"weights_v{WEIGHTS_VERSION}")
    ...
    def predict(self, images: List[np.ndarray]) -> List[list]:
        ...
        boxes = YOLOv4Tiny.postprocess_detection_results(output, self.NMS_thresh, self.conf_thresh)
        boxes = self.rescale_boxes(boxes, self.model.height, images[0].shape[:2])
